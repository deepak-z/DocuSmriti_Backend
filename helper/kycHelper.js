...
export async function saveUserKycInfo(req) {
    ...
    if(kycInfo == null){
        ...
        const err = await updateUserKycInfo(user.id,userKycInfoObject)
        if (err != null) {
    ...
    if(status == 200 && response["result"]["is_otp_sent"]){
        ...
        const err = await updateUserKycInfo(user.id,data)
        if (err != null) {
...
export async function verifyUserKycInfo(req){
    ...
    if(status == 200 && response["success"] && (response["result"] != null)){
        ...
        var data = {
            status: kyc_info.IN_PROGRESS,
            selfie_string: response["result"]["user_profile_image"]
        }

        if(kycChecksErr != null) {
            data.status = kyc_info.REJECTED
            data.rejection_reason = kycChecksResponse
        }

        const err = await updateUserKycInfo(kycInfo.user_id,data)
        if(err != null){
...
export async function verifyUserSelfie(req) {
    ...
    if(status == 200 && response["success"]){
        ...
        var data = {
            status: kyc_info.VERIFIED,
            selfie_match_score: parseFloat(response["result"]["face_match_score"])
        }

        if (response["response_code"]!= 100 || data.selfie_match_score < constant.SELFIE_MATCH_SCORE_THRESOLD) {
            data.status = kyc_info.REJECTED
